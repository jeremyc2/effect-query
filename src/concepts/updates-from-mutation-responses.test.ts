...
test("updates from mutation responses can write directly into cached query data", () =>
	Effect.runPromise(
		Effect.gen(function* () {
			...
			const renameUser = createMutationAtom({
				...
				onSuccess: (name, input) =>
					userQuery
						.setData(input.id, `${input.id}:${name}`)
						.pipe(Effect.asVoid),
			});
			...
			yield* waitForMutationSuccess(registry, renameUser);
